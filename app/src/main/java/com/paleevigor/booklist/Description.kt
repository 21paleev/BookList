...
// ветка linear_layout
// Делаем отображение списка на LL (пробно)
// В макете добавляем скролл с LL
// В main добавляем переменную LL и метод показа списка
// добавляем вызов списка в обсервер VM
// в репозиторииImpl сортируем список по ID, чтобы не менялся порядок отображения

// ветка view_holder
// создаем класс адаптера с классом VH внутри

// ветка bag_vh
// устанавливаем адаптер в активити
// если каждый элемент отображается по разному от условия,
// то показано как переиспользуемый элемент устанавливать в начальное условие
// иначе все переиспользуемые элементы станут одинаковые при скролле